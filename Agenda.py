...
#Essa função lê a quantidade de contatos:
def qto_contatos():
    print(f"O total de registros é: {[len(agenda)]}")

#Essa função lê os contatos e diz a posição que ele está:
def posicao_contato():
    print(f"Tamanho da agenda: {len(agenda)}")
    for i, contato in enumerate(agenda):
        nome = contato[0]
        telefone = contato[1]
        print(f"Posição: {i} || Nome: {nome} ||Telefone: {telefone}")
